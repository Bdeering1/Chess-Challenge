...
public class PstPacker
{
	...
	{
		decimal[] packedData = new decimal[64]; //since 64 squares on a chess board

		for (var square = 0; square < packedData.Length; square++)
		{
			byte[] packedSquares = new byte[tablesToPack.Count];

			for (var table = 0; table < tablesToPack.Count; table++)
			{
				sbyte valueToPack = (sbyte)Math.Round(tablesToPack[table][square] / 2m);
				packedSquares[table] = (byte)(valueToPack & 0xFF);
			}

			int[] thirds = new int[4]; // (because a decimal requires 16 bytes)
			for (int i = 0; i < 3; i++)
			{
				//each int stores 4-bytes of the value for this square
				//(we only use 3 ints since we have 12 tables which are 1 byte each)
				thirds[i] = BitConverter.ToInt32(packedSquares, i * 4);
			}
			//turns into decimal with 12 bits of data and 4 empty bits (ex. 0x112233445566778899AABBCC00000000)
			//(last 4 bytes un-used, since only 12 1-byte tables of 16 available bytes)
	...
	private static void UnpackData(decimal[] tablesToUnpack)
	{
		int[] piece_val = { 0, 100, 317 /* 325 - 8 */, 333 /* 325 + 8 */, 558 /* 550 + 8 */, 1000, 0 };
		int piece = 1;
		var table = tablesToUnpack.Select(packedTable =>
		new BigInteger(packedTable).ToByteArray().Take(12)
					.Select(square => (int)((sbyte)square * 2)/* + piece_val[piece++ % 6]*/)
				.ToArray()
		).ToArray();

		for (var i = 0; i < 64; i++)
		{
			int[] correct_vals = {mg_pawn_table[i], mg_knight_table[i], mg_bishop_table[i], mg_rook_table[i], mg_queen_table[i], mg_king_table[i], eg_pawn_table[i], eg_knight_table[i], eg_bishop_table[i], eg_rook_table[i], eg_queen_table[i], eg_king_table[i] };
			Console.WriteLine($"compressed: [{string.Join(", ", table[i])}],\noriginal:   [{string.Join(", ", correct_vals)}]\n");
		}
	}
}